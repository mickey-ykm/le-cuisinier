...
export const RecipeCard: React.FC<Props> = ({ recipe, language, apiKey }) => {
  ...
  const handleImageError = async () => {
    ...
    // Attempt to recover image using Gemini Search tool
    const newUrl = await findDishImage(recipe.name, apiKey);
    if (newUrl) {
      setImageUrl(newUrl);
